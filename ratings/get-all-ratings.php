...
if ($_SERVER['REQUEST_METHOD'] == "GET") {
    ...
    $sql = $obj->select("ratings", "ratings.*, users.`username`, users.`avatar`, products.name as product_name", "users JOIN products", "ratings.`user_id` = users.`id` and ratings.`product_id` = products.`id`", $conditionString, "",  $pagination);
    $result = $obj->getResult();
    if ($sql) {
        $pageInfo = array();
        $total = $obj->getResult($obj->select("ratings", "COUNT(*)", "users", "ratings.`user_id` = users.`id`", $conditionString, "",  ""));
